...
function parseFlags(argv: string[]): Flags {
  const flags: Flags = { yes: false, dryRun: false, skipContracts: false, skipFrontend: false }
...
function writeGithubEnv(config: DeployConfig, network: NetworkConfig, state: ReturnType<typeof loadState>): string {
  const path = resolve(REPO_ROOT, '.github', `env.${network.key}`)
  const a = state.addresses
  const lines = [
    '# Generated by `npm run setup` — consumed by .github/workflows/deploy.yml.',
    `VITE_NETWORK=${network.key}`,
...
async function phasePublish(
  ...
  const printManual = () => {
    ...
    ui.log(c.dim('   1) add repo secrets: DEPLOYER_SEED, DOTNS_MNEMONIC'))
    ui.log(c.dim(`   2) Actions → "Deploy" → Run workflow (network: ${network.key})`))
    ui.log(c.dim(`      or:  gh workflow run deploy.yml -f network=${network.key}`))
  }
  ...
  if (hasGh && !flags.yes) {
    ...
    const run = spawnSync('gh', ['workflow', 'run', 'deploy.yml', '-f', `network=${network.key}`], { encoding: 'utf8' })
    if (run.status === 0) {
