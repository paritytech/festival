...
<template>
  <div
    ...
    class="rounded-2xl bg-bg-surface-nested px-6 pt-6 pb-5 flex flex-col items-center text-center"
  >
    <div v-if="badgePixels" class="w-[52%] aspect-square rounded-2xl overflow-hidden">
      <BadgeCanvas :pixels="badgePixels" :size="128" />
    </div>

    <p class="text-[18px] leading-[22px] font-semibold text-text-and-icons-primary mt-5">{{ displayName }}</p>

    <p class="text-xs text-text-and-icons-tertiary mt-2">
      {{ startTime }} - {{ endTime }}
